    ...
    public class GeneralSimplifier
    {
        ...
        private AstIdx? UnmergeDisjointParts(AstIdx withSubstitutions, Dictionary<AstIdx, AstIdx> substitutionMapping, ref bool isSemiLinear)
        {
            ...
            for (int i = 0; i < results.Count; i++)
            {
                ...
                var resultVec = LinearSimplifier.JitResultVector(ctx, ctx.GetWidth(expr), moduloMask, vars, expr, multiBit: true, numCombinations);
                var constantOffset = LinearSimplifier.SubtractConstantOffset(resultVec, (int)numCombinations, w, multiBit: true);
        ...
        private (AstIdx? semilinearIdx, AstIdx? unconstrainedIdx, AstIdx? constrainedIdx) PartitionConstrainedParts(AstIdx withSubstitutions, Dictionary<AstIdx, AstIdx> substitutionMapping)
        {
            ...
            var constantOffset = LinearSimplifier.SubtractConstantOffset(resultVec, (int)numCombinations, bitSize, multiBit: true);
